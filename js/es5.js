...
function () {
  ...
  _createClass(Lucky, [{
    ...
    value: function stop() {
      ...
      this.numPwrap.innerHTML = "<span class=\"current-lucky-num\">".concat(this.luckyNum, "</span>");
    } // 滚动音效
        ...
        if (_this4.oLocalStorage.getItem(i)) {
          ...
          if (_this4.isShowTurn) {
            allData.map(function (item) {
              if (item) {
                _this4.numPwrap.firstChild.innerHTML += "<div class=\"show\"><span class=\"turnShow\">\u7B2C".concat(i, "\u8F6E</span>").concat(item.title, "\uFF1A").concat(item.num, "</div>");
              }
            });
          } else {
            allData.map(function (item) {
              if (item) {
                _this4.numPwrap.firstChild.innerHTML += "<div class=\"show\">".concat(item.title, "\uFF1A").concat(item.num, "</div>");
              }
            });
          }
        }
